...
var books_f = function() {
	...
	f.read = function (fields, query, limit, offset, lat, lng, radius, callback) {
		if(lat && lng && radius) {
			console.log('search with loc');
			var area = { center: [lng, lat], radius: radius };
			var re = new RegExp(query, 'i');
			Books.find({ $or: [{title: re},{author: re}], num_users: { $gt: 0 } }).where('loc.coordinates').within.center(area).select(fields).skip(offset || 0).limit(Number(limit) || 10).exec(function(err, data) {console.log(err + data);callback(err, data);});
		} else {
			var re = new RegExp(query, 'i');
