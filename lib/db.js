...
var users_f = function() {
	...
	f.queryBooks = function (user, data, callback) {
		User.findOne({email:user}, 'library').populate('library.id', null, 'BookModel').exec(function (err, data) {
			if(!err && user) {
				var mybooks = [];
	...
	f.removeBook = function (user, book, callback) {
		User.findOne({email:user}, 'library').populate('library.id', null, 'BookModel').exec(function (err, data) {
			if(!err && user) {
				for (var i = 0; i < data.library.length; i++) {
	...
	f.addBook = function(user, book_id, callback) {
		User.findOne({email: user}, function (err, user) {
			if(!err && user && book_id) {
...
var books_f = function() {
	...
	f.add = function(this_user, data, callback) {
		...
		if(data.title && data.author) {
			...
			users.findId(this_user, function(id) {
				...
				book.save();
				users.addBook(this_user, book._id);
				callback(true);
			});
		} else {
			callback(false);
	...
	/*
	f.removeUser = function(this_user, data, callback) {
		users.findOne(this_user, function(id) {
			if(!err id) {
	...
	*/
	f.query = function(callback) {
		Book.find({ users: {$exists: true, $size: "1"}}, function(err, books) {
			callback(books);
		});
	};
