...
var users_f = function() {
	...
	f.queryBookss = function (user, data, callback) {
		Users.findOne({email:user}, 'library').populate('library.id', null, 'BooksModel').exec(function (err, data) {
			if(!err && user) {
				var mybooks = [];
	...
	f.authorize = function (id, token, callback) {
		Users.findById(id, token, function (err, user) {
			if(!err && user && user.token.hash == key) {
				if(callback) callback(false);
			}
			else {
				if(callback) callback(true);
