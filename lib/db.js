...
var users_f = function() {
	...
	f.findMailByUsername = function (user, callback) {
		if(user) {
			Users.findOne({username: new RegExp("^" + user + "$", "i") }, 'email').exec(callback);
		}
		else callback(true);
	};
	...
	f.exists = function (email, username, callback) {
		valid.email(email, function(err) {
			if(err) {callback(true, 'invalidEmail'); return;}
			valid.email(username + "@bibliofair.com", function(err) {
				if(err) {callback(true, 'invalidUsername'); return;}
		...
		Users.findOne({ $or: [{email: id},{username: id}]}, function (err, user) {
			if(user && !err) {
				if ( ( user.email == id && user.password.email == password ) || ( user.username == id && user.password.username == password) ) {
					...
					user.token.last = new Date();
					user.save();
					callback(false, user._id, token, user.language);
				}
				else { callback(true);}
			}
			else {
...
var library_f = function () {
			...
			if(!err && user && data.author && data.title && user.loc.coordinates.length == 2) {
				...
				Books.find(newbook).limit(1).exec(function(err, book) {
					if(!err && book.length == 1) {
						...
						book[0].save();
						user.library.push({id:book[0]._id, last_updated: new Date(), actions: data.actions});
						user.save();
						...
						console.log(user.loc.coordinates);
						book.loc.push({coordinates: user.loc.coordinates});
						book.num_users = 1;
						book.save();
						console.log(user.library);
						user.library.push({id:book._id, last_updated: new Date(), actions: data.actions});
						console.log(user.library);
						user.save();
						callback(false, book);
					}
	...
	f.update = function (id, book_id, actions, callback) {
		console.log(actions);
		Users.findById(id, function(err, user) {
			if(!err && user.library) {
				for(var i = 0; i < user.library.length; i++) {
					if(user.library[i].id == book_id) {
						user.library[i].actions = actions;
					}
				}
				user.save();
				callback(false);
...
var books_f = function() {
	...
	f.read = function (fields, query, limit, offset, lat, lng, radius, callback) {
		if(lat && lng && radius) {
			console.log('search with loc' + lat + " " + lng + " " + radius);
			...
			Books.find({ $or: [{title: re},{author: re}], num_users: { $gt: 0 } }).where('loc.coordinates').near({ center: [lng, lat], maxDistance: (radius/111.12), spheriacal: true}).select(fields).skip(offset || 0).limit(Number(limit) || 12).exec(function(err, data) {console.log(err);callback(err, data);});
		} else {
			var re = new RegExp(query, 'i');
			Books.find({ $or: [{title: re},{author: re}], num_users: { $gt: 0 } }).select(fields).skip(offset || 0).limit(Number(limit) || 12).exec(callback);
		}
	};
	...
	f.count = function (callback) {
		Books.count({}, function(err, count) { callback(err, count);});
	};
	return f;
};
var books = books_f();
