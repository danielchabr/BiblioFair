...
var books_f = function() {
	...
	f.read = function (fields, query, limit, offset, lat, lng, radius, callback) {
		if(lat && lng && radius) {
			...
			Books.find({ $or: [{title: re},{author: re}], num_users: { $gt: 0 } }).where('loc.coordinates').near({ center: [lng, lat], maxDistance: (radius/111.12), spheriacal: true}).select(fields).skip(offset || 0).limit(Number(limit) || 12).populate('users', {"library":1, _id:0}, 'UserModel').exec(function(err, data) {console.log(err);callback(err, data);});
		} else {
			var re = new RegExp(query, 'i');
