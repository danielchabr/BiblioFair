...
var users_f = function() {
	var f = {};
	f.get = function (id, fields, callback) {
		Users.findById(id, ['username', 'loc', 'library'].join(' ')).select(fields.join(' ')).exec(callback);
	};
...
var books_f = function() {
	...
	f.read = function (query, limit, offset, callback) {
	};
