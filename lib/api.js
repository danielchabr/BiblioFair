...
module.exports = function(err, db, req, res) {
	...
	if(path[0] == 'books') {
		books(db, req, res, path.shift(), auth);
	}
	else if(path[0] == 'library') {
		library(db, req, res, path.shift(), auth);
	}
	else if(path[0] == 'users') {
		users(db, req, res, path.shift(), auth);
	}
	else if(path[0] == 'tel') {
		tel(req, res, path.shift(), auth);
	}
	else {
		res.status(400).send();
	}
...
var library = function(db, req, res, path, auth) {
	if(auth) {
		if(req.method == 'GET') { // READ
			db.library.read(req.query.id, function (err, data) {
				if(!err) {
					res.send(data);
				} 
				else {
				...
				db.library.create(req.body.id, req.body.book, function (err) {
					if(!err) {
						res.send('OK');
					} 
					else {
...
var users = function(db, req, res, path, auth) {
	if(auth) {
		if(req.method == 'GET') { // READ
			db.users.read(req.query.id, function (err, data) {
				if(!err) {
					res.send(data);
				} 
				else {
