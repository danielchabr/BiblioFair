...
/////////////  PUBLIC PART
	...
	app.all("/*", function(req, res, next) {
		db.users.auth(req.cookies.user, req.cookies.token, function(err) {
		if(err) {
			if(req.path == '/') {
				res.sendfile(public_path + '/index.html');
			}
...
////////////   PRIVATE PART 
	...
	app.all('/api/:name/:service/:operation', function (req, res, next) {
		if(req.params.name && req.params.service && req.params.operation) {
			db.users.auth(req.params.name, req.cookies.token, function(err) {
				if(err) next();
				else
				api(db, req.params.name, req.params.service, req.params.operation, req.body, function (err, resp) {
					if(err) res.send(err);
